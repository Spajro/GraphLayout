...
fn test() {
    ...
    state.positions.insert(Vertex { id: 2 }, Position { x: 100, y: 300 });
    state.positions.insert(Vertex { id: 3 }, Position { x: 300, y: 100 });
    state.positions.insert(Vertex { id: 4 }, Position { x: 300, y: 300 });
    println!("WORK");
    for _i in 0..5 {
        display(state);
        iterate(state);
